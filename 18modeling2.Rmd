...
```{r mtcars-median-imputation-example, cache=TRUE}
data("mtcars")
set.seed(42)
...
```{r mtcars-knn-imputation-example, cache=TRUE}
...
```{r mtcars-multiple-preprocessing-example, cache=TRUE}
data("mtcars")
set.seed(42)
mtcars[sample(1:nrow(mtcars), 10), "hp"] <- NA
Y <- mtcars$mpg
...
```{r traincontrol-reusing-example, cache=TRUE}
library(C50)
library(modeldata)
data(mlc_churn)
set.seed(1)
...
So the test and training sets have approximately equal proportions of churn. 
