...
Note that the RMSE in this case is $`r sqrt(mean(error^2))`, which is in keeping with what we know about the price of diamonds, which range from $`r range(diamonds$price)[1]` to $`r range(diamonds$price)[2]` and average $`r round(mean(diamonds$price), 2)`.
