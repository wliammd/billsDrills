...
An example: to cite a breast pathology paper that I want to cite here [@RN2750], I'd copy-and-paste the reference from `bookFromEndnote.txt` to `book.bib` or grab the `BibTex Export` from EndNote directly. 
...
All packages are maintained in `packages.bib`. **bookdown**, for instance, is cited thusly ``[@R-bookdown]`` [@R-bookdown]. Individual references can be strung together with semicolons [@R-bookdown; @R-base; @RN5924].
