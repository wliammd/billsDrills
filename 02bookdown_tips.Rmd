...
Note that RStudio offers a "Preview in Viewer Pane" option, which is vastly faster and more convenient (although smaller) than previewing your beautiful file in a standalone window. When you have the luxery of working with a large monitor, this is the best way to watch your code unfold into an Rmarkdown document. This is found under the Preferences menu, R Markdown submenu. 
...
It it sometimes difficult to keep track of all the packages used in a large bookdown project. The following blog post is a good place to start <https://statsandr.com/blog/an-efficient-way-to-install-and-load-r-packages/>.

A package by Desi Quintans may help: **librarian** may be just the thing that I need. Desi Quintans describes it in this post on CRAN <https://cran.r-project.org/web/packages/librarian/vignettes/intro-to-librarian.html>.
...
```{r}
# Package names
packages <- c("tidyverse", "knitr", "formatR", "bookdown", "tufte", "forcats", "gapminder", "skimr", "ggridges", "personograph", "assertive", "plotly", "ggfortify", "Rtsne", "tidycensus", "caret", "rpart", "MASS", "caTools", "C50", "modeldata", "credentials", "MLmetrics", "randomForest", "googlesheets", "googlesheets4")
