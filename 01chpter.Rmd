...
In order to insert reference citations, one needs a  `.bib` file in the project. I've included one in this project as `book.bib`. The yml header in Chapter \@ref(intro) needs to have a $bibliography:$ and $biblio-style:$ line added. A separate `packages.bib` file is also used to hold the R packages cited.
